	    ...
	    function tc_colophon_center_block() {
	    	echo apply_filters(
	    		...
	    		sprintf('<div class="%1$s">%2$s</div>',
		    		...
		    		sprintf( '<p>%1$s %2$s</p>',
						    apply_filters( 'tc_copyright_link', sprintf( '&copy; %1$s <a href="%2$s" title="%3$s" rel="bookmark">%3$s</a>', esc_attr( date( 'Y' ) ), esc_url( home_url() ), esc_attr( get_bloginfo() ) ) ),
						    apply_filters( 'tc_credit_link', sprintf( 'Designed by %1$s ', 'Niamherinoc' ) )
					)
	    		)
	    	);
	    }
