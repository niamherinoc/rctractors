...
### New Conversion Array Replace CONV
$bbld_unwanted = array('Š'=>'S', 'š'=>'s', 'Ž'=>'Z', 'ž'=>'z', 'À'=>'A', 'Á'=>'A', 'Â'=>'A', 'Ã'=>'A', 'Ä'=>'A', 'Å'=>'A', 'Æ'=>'A', 'Ç'=>'C', 'È'=>'E', 'É'=>'E', 'Ê'=>'E', 'Ë'=>'E', 'Ì'=>'I', 'Í'=>'I', 'Î'=>'I', 'Ï'=>'I', 'Ñ'=>'N', 'Ò'=>'O', 'Ó'=>'O', 'Ô'=>'O', 'Õ'=>'O', 'Ö'=>'O', 'Ø'=>'O', 'Ù'=>'U', 'Ú'=>'U', 'Û'=>'U', 'Ü'=>'U', 'Ý'=>'Y', 'Þ'=>'B', 'ß'=>'Ss', 'à'=>'a', 'á'=>'a', 'â'=>'a', 'ã'=>'a', 'ä'=>'a', 'å'=>'a', 'æ'=>'a', 'ç'=>'c', 'è'=>'e', 'é'=>'e', 'ê'=>'e', 'ë'=>'e', 'ì'=>'i', 'í'=>'i', 'î'=>'i', 'ï'=>'i', 'ð'=>'o', 'ñ'=>'n', 'ò'=>'o', 'ó'=>'o', 'ô'=>'o', 'õ'=>'o', 'ö'=>'o', 'ø'=>'o', 'ù'=>'u', 'ú'=>'u', 'û'=>'u', 'ý'=>'y', 'ý'=>'y', 'þ'=>'b', 'ÿ'=>'y' );
...
function wpbb_permalink($type,$topicid, $slug = '',$total = 0,$limit = 30, $forumslug = '') {
	global $wpdb,$BbLD_version,$bbld_unwanted;
	$slug = strtr($slug, $bbld_unwanted);
	$forumslug = strtr($forumslug, $bbld_unwanted);
...
function bbld_getforum($widget = 0) {
	...
	if ($bbforum) {
		foreach ($bbforum as $forum) {
			...
			$forum_name = strtr($forum->forum_name, $bbld_unwanted);
			$forum_desc = strtr($forum->forum_desc, $bbld_unwanted);
...
class bbld_forum_widget extends WP_Widget {
	function bbld_forum_widget() {
		$widget_ops = array('classname' => 'bbld_forum_widget', 'description' => __("BbLD Forum Widget") );
		$this->WP_Widget('bbld_forum_widget', __('BbLD Forum Widget'), $widget_ops);
	}
...
class bbld_widget extends WP_Widget {
	function bbld_widget() {
		$widget_ops = array('classname' => 'bbld_widget', 'description' => __("BbLD Widget") );
		$this->WP_Widget('bbld_widget', __('BbLD Widget'), $widget_ops);
	}
	function widget( $args, $instance ) {
		extract($args);
		$bbld_option = get_option('bbld_option');
		$title = apply_filters('widget_title', $instance['title']);
		echo $before_widget;
		if ( !$title ) { $title = 'Forum Last %BBLD_LIMIT% Discussions'; }
		$title = str_replace("%BBLD_LIMIT%", $bbld_option['limit'], $title);
		echo $before_title . $title . $after_title;
		echo '<ul>';
		bbld_getside();
		echo "</ul>";
	...
	function form( $instance ) {
		...
		$title = $instance['title'];
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo esc_attr($title); ?>" /><br /><span class="setting-description">Allowed Variables: %BBLD_LIMIT%</span></label></p>
<?php
	}
	function update( $new_instance, $old_instance ) {
		$instance = $old_instance;
...
function bbld_add_widget() {
	register_widget('bbld_widget');
	register_widget('bbld_forum_widget');
}

### Function: Add BbLD Widget
add_action('widgets_init', 'bbld_add_widget');
